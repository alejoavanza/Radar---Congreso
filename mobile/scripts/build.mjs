...
const python = process.env.RADAR_PYTHON || (existsSync(localPython) ? localPython : 'python3');
let html = execFileSync(python, [join(mobile, 'scripts/render-template.py')], {encoding: 'utf8'});
...
if (scripts.length !== 5) throw new Error('The source script list changed; review bootstrap order before building.');
...
<p>Se conserva el último reporte y el último comparativo en este dispositivo. Las búsquedas nuevas requieren internet.</p>
