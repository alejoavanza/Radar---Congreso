...
Fecha: 23 de septiembre de 2026. Revisión técnica del código de Radar Político. No es la declaración presentada a Apple ni una certificación de las prácticas de los proveedores.

## Flujo observado

| Información | Tratamiento en esta versión | Código |
|---|---|---|
| Nombre, términos asociados, zona y periodo | Se envían desde iPhone y Android al servidor de Radar para producir el reporte. El servidor construye consultas para Google Noticias, DuckDuckGo y las páginas públicas de los resultados. | `mobile/src/transport.js`, `app.py` |
| Congresistas seleccionados e intervalo | Se envían al servidor. Comparativos usa las mismas fuentes web; no llama a redes sociales. | `static/comparisons.js`, `comparisons.py` |
| Último reporte, comparativo y pestaña | Se conservan en el dispositivo mediante Preferences, junto con evidencia temporal de búsquedas. El botón de borrado elimina la copia guardada de la app. | `mobile/src/storage.js`, `mobile/src/main.js` |
| Resultados de comparativos | Las respuestas se reutilizan en memoria del servidor durante hasta 5 minutos; la evidencia reciente tiene vigencia de hasta 24 horas. Las cachés son acotadas y no son una base de datos duradera. | `comparisons.py`, `search_state.py`, `static/search-session.js` |
...
| Contenido compartido | El usuario decide compartir un resumen y enlaces mediante la hoja nativa del dispositivo. | `mobile/src/share.js`, `mobile/src/main.js` |
| Sitios abiertos | El usuario abre fuentes y perfiles en Safari integrado (iOS) o Custom Tabs (Android); el sitio de destino aplica sus prácticas. | `mobile/src/main.js` |
...
| Proveedores de fuentes | Condiciones de uso y tratamiento aplicables a las consultas de Google Noticias, DuckDuckGo y los sitios consultados, incluidos permisos para el uso dentro de la app. | Responsable técnico y Táctika |
