...
export function sourceURL(value, localOrigin) {
  try {
    const url = new URL(value);
    // Android serves bundled pages from https://localhost; keep them inside the app.
    if (localOrigin && url.origin === localOrigin) return null;
    return ['https:', 'http:'].includes(url.protocol) && !url.username && !url.password ? url.href : null;
  } catch { return null; }
}

// Native HTTP is restricted to the existing JSON endpoints. Bundled assets
// use the platform's local loader; arbitrary URLs never reach this bridge.
export function createRequest(http, connected = () => true) {
  return async function request(path, options = {}) {
    const method = options.method || 'GET';
    const windowRequest = path === '/api/search/window' && method === 'GET';
    if (!windowRequest && (!API_PATHS.has(path) || method !== 'POST')) throw new Error('Consulta no permitida.');
    ...
    const data = windowRequest ? undefined : JSON.parse(options.body || '{}');
    let timer, onAbort;
