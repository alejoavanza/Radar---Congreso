...
async function start() {
  if (!Capacitor.isNativePlatform()) throw new Error('Abre este proyecto desde la aplicación de Radar Político.');
  ...
  button('Borrar consultas guardadas', 'native-clear-saved', $('native-settings'), async () => {
    if (!window.confirm('¿Borrar las consultas guardadas en este dispositivo?')) return;
  ...
  document.addEventListener('click', event => {
    ...
    if (!anchor) return;
    const url = sourceURL(anchor.href, window.location.origin);
    if (!url) return; // Bundled help/privacy pages retain their own return link.
  ...
  if (Capacitor.getPlatform() === 'android') {
    // The app has a light canvas regardless of the Android system theme.
    await SystemBars.setStyle({style: SystemBarsStyle.Light});
    await App.addListener('backButton', async () => {
      // This listener runs only on Radar's root page. Returning from a local
      // help page may leave WebView history, which must not reopen that page.
      if ($('compare-tab').getAttribute('aria-selected') === 'true') {
        window.showTab('report'); return;
      }
      window.saveReport?.();
      await storage.flush();
