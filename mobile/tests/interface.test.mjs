...
test('packaged Radar queries the native API, restores offline, and keeps the result after a failed refresh', async () => {
  ...
  const report = {name: 'Persona de prueba', days: 1, summary: 'Reporte de prueba', items: [{title: 'Fuente de prueba', url: 'https://example.test/article', published:new Date(Date.parse(end)-3600000).toISOString(), sentiment: 'Neutral', source: 'Medio'}], mentions: {web: 1}, web_coverage:{status:'available',limited:false}};
  const first = await open(storage, async (path, options) => {
    if (path === '/api/search/window') return {ok:true,json:async()=>({end_time:end})};
    calls++;
