...
test('the shared cutoff uses the allowlisted GET endpoint and rejects other methods and hosts', async () => {
  let sent;
  const request = createRequest({request:async value => {sent=value;return {status:200,data:{end_time:'2026-09-23T12:00:00Z'}};}});
  assert.equal((await request('/api/search/window',{cache:'no-store'})).ok,true);
  assert.equal(sent.method,'GET');
  assert.equal(sent.data,undefined);
  assert.equal(sent.url,API_ORIGIN+'/api/search/window');
  await assert.rejects(request('/api/search/window',{method:'POST'}),/no permitida/);
  await assert.rejects(request('/api/report'),/no permitida/);
  assert.equal(sourceURL('https://localhost/privacy.html','https://localhost'),null);
  assert.equal(sourceURL('https://localhost.evil.example/news','https://localhost'),'https://localhost.evil.example/news');
});

test('an oversized evidence cache cannot prevent saving or deleting the last report', async () => {
  const prefs=preferences(), storage=await createStorage(prefs);
  storage.setItem('radar:evidence:v1','x'.repeat(3*1024*1024));
  storage.setItem('radar:report:v1','new report');
  await storage.flush();
  const restored=await createStorage(prefs);
  assert.equal(restored.getItem('radar:report:v1'),'new report');
  assert.equal(restored.getItem('radar:evidence:v1'),null);
